...
def PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c4, wells_coord, CP_dict, delta_r):
...
if __name__ == '__main__':
    PinPoint = []
    for t in range(T_exp1):
        print(t)
        Pres_distrib, A_full, B = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c4, wells_coord, CP_dict, delta_r)
    ...
    wells_coord = [(round(0.1392 / delta_r), round((np.pi + 1.1489) / delta_fi))]
    P_well = [100000]
    print(wells_coord)
    ...
    def sortByAngle(inputSet):
        return inputSet[1]
    PinPoint = []
    for t in range(T_exp2):
        print(t)
        Pres_distrib, A_full, B = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c4, wells_coord,
                                                       CP_dict, delta_r)
        print(min(Pres_distrib.flat), max(Pres_distrib.flat))
        PinPoint.append(Pres_distrib[2][2])
